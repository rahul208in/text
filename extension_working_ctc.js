...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('sss.generateReports', () => {
        ...
        if (projectType === 'JavaScript') {
            ...
            reportContent.violations = checkDependencies(packageJson, ruleJson.libraries);
        ...
        const reportFileName = `ctcscan_report_${Date.now()}.json`;
        const reportFilePath = path.join(resultFolderPath, reportFileName);
        fs.writeFileSync(reportFilePath, JSON.stringify(reportContent, null, 4));
